# app.py – FINAL, PERFECT, WORKING VERSION (Dec 2025)
...
if st.session_state.get("demo"):
    demand_df = pd.DataFrame({
        ...
        "store_id": np.random.choice(["North", "South", "East", "West"], 50),
        "product_id": np.random.choice([f"P{str(i).zfill(4)}" for i in range(1, 21)], 50),
        "units_sold": np.random.randint(10, 120, 50),
        "price": np.round(np.random.uniform(199, 3999, 50), 2)
    })
    retail_df = None
    st.success("Demo data loaded – running automatically!")
    run_now = True
else:
    run_now = False
...
# -------------------------------------------------
# 3. RUN BUTTON (only if not in demo mode)
# -------------------------------------------------
if not st.session_state.get("demo") and demand_file:
...
# 4. RUN THE ENGINE
# -------------------------------------------------
if run_now and 'demand_df' in locals():
    with st.spinner("Analyzing data & calculating optimal prices..."):
    ...
    st.dataframe(results_df.sort_values("revenue_uplift_%", ascending=False), use_container_width=True, hide_index=True)
    ...
    ax2.axvline(row["optimal_price"], color="red", linestyle="--", lw=3, label=f"Optimal = ₹{row['optimal_price']}")
    ax2.legend(); ax2.set_title("Revenue Curve")
    st.pyplot(fig)

    st.markdown("**Your data was processed in memory only – never stored.**")
...
# -------------------------------------------------
    ...
    st.info("""
    Click the big red button above for an instant demo!

    Or upload your own CSV with columns like:
    • Date • Product ID • Store ID • Units Sold • Price
    The AI auto-detects everything.
    """)
    ...
    with col1:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Store ID": ["S001", "S002"],
            "Product ID": ["P0018", "P0018"],
            "Units Sold": [67, 82],
            "Price": [82.97, 91.27]
        }))
    with col2:
        st.markdown("**Your file can have 100+ columns – we only need 5**")
